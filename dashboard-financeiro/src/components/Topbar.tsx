                ...
                <div className="flex w-full items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            autoFocus
                            className="w-full bg-accent/50 border border-border/50 rounded-lg pl-10 pr-10 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary h-10"
                        />
                        {searchQuery && (
                            <button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2" title="Limpar">
                                <X className="h-4 w-4 text-muted-foreground" />
                            </button>
                        )}
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => setShowMobileSearch(false)}>
                        <h2 className="text-sm font-medium">Cancelar</h2>
                    </Button>
                </div>
            ) : (
                <>
